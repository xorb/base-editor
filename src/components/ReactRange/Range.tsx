...
class Range extends React.Component<IProps> {
    ...
    return this.getThumbs().map((thumb, index) => {
      ...
      switch (direction) {
        case Direction.Right:
          ...
          thumbOffsets.x += trackRect.width * relativeValue(values[index], min, max) - thumbRect.width / 2
          return thumbOffsets
        case Direction.Left:
          thumbOffsets.x = (thumbMargins.right + trackPadding.right) * -1
          thumbOffsets.y = ((thumbRect.height - trackRect.height) / 2 + trackPadding.top) * -1
          thumbOffsets.x +=
            trackRect.width - trackRect.width * relativeValue(values[index], min, max) - thumbRect.width / 2
          return thumbOffsets
        case Direction.Up:
          thumbOffsets.x = ((thumbRect.width - trackRect.width) / 2 + thumbMargins.left + trackPadding.left) * -1
          thumbOffsets.y = -trackPadding.left
          thumbOffsets.y +=
            trackRect.height - trackRect.height * relativeValue(values[index], min, max) - thumbRect.height / 2
          return thumbOffsets
        case Direction.Down:
          thumbOffsets.x = ((thumbRect.width - trackRect.width) / 2 + thumbMargins.left + trackPadding.left) * -1
          thumbOffsets.y = -trackPadding.left
          thumbOffsets.y += trackRect.height * relativeValue(values[index], min, max) - thumbRect.height / 2
          return thumbOffsets
        default:
  ...
  getThumbs = () => {
    if (this.trackRef && this.trackRef.current) {
      return Array.from(this.trackRef.current.children).filter((el: any) => el.hasAttribute("aria-valuenow"))
    }
    console.warn(
  ...
  onKeyDown = (e: React.KeyboardEvent) => {
    ...
    const inverter = rtl || direction === Direction.Left || direction === Direction.Down ? -1 : 1
    if (index === -1) return
    ...
    if (INCREASE_KEYS.includes(e.key)) {
      ...
      onChange(
        replaceAt(
          values,
          index,
          this.normalizeValue(values[index] + inverter * (e.key === "PageUp" ? step * 10 : step), index)
        )
      )
      ...
      onChange(
        replaceAt(
          values,
          index,
          this.normalizeValue(values[index] - inverter * (e.key === "PageDown" ? step * 10 : step), index)
        )
      )
    } else if (e.key === "Tab") {
  ...
  onMove = (clientX: number, clientY: number) => {
    ...
    if (draggedTrackPos[0] !== -1 && draggedTrackPos[1] !== -1) {
      ...
      if (Math.abs(values[draggedThumbIndex] - newValue) >= step / 2) {
        onChange(replaceAt(values, draggedThumbIndex, this.normalizeValue(newValue, draggedThumbIndex)))
      }
    }
  }
  ...
  render() {
    const { renderTrack, renderThumb, renderMark = () => null, values, min, max, allowOverlap, disabled } = this.props
      ...
      children: [
        ...markOffsets.map((offset, index, arr) =>
          renderMark({
            props: {
              style:
                this.props.direction === Direction.Left || this.props.direction === Direction.Right
                  ? {
                      position: "absolute",
                      left: `${offset[0]}px`,
