...
class ObjectImporter {
  ...
  async import(item: ILayer, options: Required<ILayer>, inGroup: boolean = false): Promise<fabric.Object> {
    ...
    switch (item.type) {
      ...
      //   object = await this.separator(item, options, inGroup)
      //   break
      // case LayerType.PLACEHOLDER:
      //   object = await this.placeholder(item, options, inGroup)
      //   break
  ...
  //   inGroup: boolean
  // ): Promise<fabric.Separator> {
  //   return new Promise(async (resolve, reject) => {
  //     try {
  //       const baseOptions = this.getBaseOptions(item, options, inGroup)
  //       const { fill, kind } = item as ISeparator

  //       const element = new fabric.Separator({
  //         ...baseOptions,
  //         fill,
  //         kind,
  //       })
