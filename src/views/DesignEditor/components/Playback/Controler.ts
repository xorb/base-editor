...
class PlaybackController {
  ...
  public render = (time: number) => {
    for (let [key, resource] of this.resources) {
      if (time > resource.display.from && time < resource.display.to!) {
        if (!resource.sprite.visible && resource.type === "StaticVideo") {
          this.displayAndPlayVideo(resource, time)
        }
  ...
  public displayAndPlayVideo = (resource: ElementWithSprite, time: number) => {
    resource.video.currentTime = resource.cut.from / 1000
    resource.sprite.visible = true
    resource.video.muted = false
    resource.video.play()
  }
  ...
  public play = (time: number) => {
    ...
    for (let [key, value] of this.resources) {
      ...
      if (value.type === "StaticVideo") {
        ...
        // value.video.currentTime = value.cut.from / 1000
        value.video.currentTime = value.cut.from / 1000

        // value.video.currentTime = time / 1000 + value.cut.from / 1000
      }
